;; import
(.add-to-load-path "~/Dropbox/Sync/emacs/jdee-2.4.0.1/lisp/")
(require 'jde)
...
semanticdb-javap-paths-for-package
...
(defmethod semanticdb-java-jar-find-file ((dbc semanticdb-java-jar-database) file-name)
  "Get the file class names from DBC that match DIR."
  (let ((ans nil))
    (dolist (F (oref dbc jarfilecache))
      (when (string-match (concat "[/$]" (regexp-quote file-name) "\\.class$")
                          F)
        (push F ans)))
    (nreverse ans)))
